...
### 根本原因の調査(2026-09-13): 「軽い復旧」は存在しない、ソフトウェアでは直せない

`/cam start` の失敗が頻発する根本原因を、再起動の手間を減らす目的で調査した。

**結論: このラズパイのBluetoothチップ(内蔵BCM4345C0、UART接続、WiFiと共有のコンボチップ)は
ファームウェアレベルで詰まる既知の不具合があり、詰まった後はOS上のどんなコマンドでも直せない。
唯一の直し方は `sudo reboot`(SoCの電源シーケンスがチップごとリセットする)。**

**検証して分かったこと:**
- `sudo systemctl restart hciuart.service`(起動時にBTファームウェアパッチを書き込むサービス)を
  稼働中のhci0に対して実行すると、**直す どころか壊れる**(`hci0: command tx timeout`が発生し、
  `hciconfig hci0 down/up` すら `Device or resource busy` で失敗、`bluetoothctl` も
  タイムアウトする)。実機で確認済み(この状態から復旧できたのは `sudo reboot` のみ)。
  → **「軽い代替手段」としてこれを自動復旧に組み込むのは危険**(逆に壊すリスクの方が高い)。
- `xaionaro-go/djictl` issue #4 でも同種の報告あり
  (「一度死ぬとreboot程度では直らないこともあり、電源の入れ直しが必要」)。
  チップのファームウェアが内部でループに陥って応答しなくなる、という推測と一致する。
- カーネルログ(`journalctl -k`)には該当期間、Bluetooth関連のエラーは一切出ない
  (=カーネルドライバではなくチップのファームウェア内部の話なので、Linux側からは見えない)。

**唯一実施した恒久対策(即時反映ではなく次回起動時から有効):**
WiFi省電力(power save)が有効(`on`)だったため、`/etc/NetworkManager/conf.d/wifi-powersave-off.conf`
に `wifi.powersave = 2`(無効化)を追加した。BCM4345はWiFi/BTでアンテナを共有するコンボチップのため、
WiFi側の省電力による頻繁なスリープ復帰がBT側のコマンドタイムアウト・不安定化を誘発する既知の要因。
無料かつ低リスクな緩和策だが、発生頻度が実際に下がるかは今後の経過観察が必要(即座に検証はできない)。

**今後さらに発生頻度を下げたい場合の本命は「外付けUSB Bluetoothドングルへの切り替え」。**
内蔵チップ自体がボトルネックなので、ソフトウェア側の工夫(watchdog調整・rfkill等)には限界がある。
ただしdjictlは現状アダプタ選択オプションを持たない(`ble --filter-device-addr` はデバイス側の
フィルタであり、ホスト側のhciアダプタ選択ではない)ため、外付けドングルを使うには
`deploy/djictl-patches/` 同様のパッチが別途必要になる可能性が高い。実施は保留中。
